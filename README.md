...
- **较大迭代后自动沉淀**:口径/评分逻辑变更、核心结论变化、重要决策或否决之后,agent 把沉淀作为该任务的最后一步主动执行,不等用户提醒;错别字、样式微调这类小改动攒到会话收尾一并处理。
...
在已有 handoff 结构的项目里,较大迭代完成后 agent 会**自动**把沉淀作为最后一步执行,不需要你每次都说"沉淀"。手动触发时它会:
